...
# COG ADDITION (2026-08-20, D27). Upstream takes the generation seed ONLY from
# env.cfg.datagen_config.seed, with no way to vary it per invocation. Our L3 levels are 10 separate
# variant envs generated by 10 separate calls to this script, so every call replayed the identical
# pose stream: the nominal 400-demo L3 datasets held just 43-48 UNIQUE initial poses (~9x
# redundancy) in all three tasks, while L1/L2 held 400. That silently turned L3's demo-count axis
# into a 10x-inflated one and produced an apparent "generality ceiling" that was really a
# pose-coverage ceiling. Default None = upstream behaviour untouched.
parser.add_argument(
    "--seed",
    type=int,
    default=None,
    help="override env.cfg.datagen_config.seed; REQUIRED to differ across the sub-variant runs of "
         "one level, or every variant regenerates the same initial poses.",
)
...
def main():
    ...
    print(f"[cog] generation seed = {env.cfg.datagen_config.seed}", flush=True)
